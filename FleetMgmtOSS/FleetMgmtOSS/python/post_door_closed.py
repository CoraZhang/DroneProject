...
# parameters_file = (sys.argv[1] if len(sys.argv) == 2 else os.environ.get('ADAL_SAMPLE_PARAMETERS_FILE'))
...
door_status = '[{"id":"8d77241f-07cc-4990-b7f5-e07ff3b773f2","v":[{"m":[0],"t":%d}]}]' % (time_now)
headers = { 'Authorization': httpSAS, 'DataCollectorId': gatewayID, 'PayloadType': 'Measurements', 'Timestamp': time_now, 'Cache-Control': 'no-cache', 'Content-Length': len(door_status) }

response = requests.post(url, data=door_status, headers=headers)
