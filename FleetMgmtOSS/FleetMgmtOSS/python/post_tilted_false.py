...
tilt_status = '[{"id":"5a0cc0cf-5bcc-4106-95c8-be52855eb145","v":[{"m":[0],"t":%d}]}]' % (time_now)
headers = { 'Authorization': httpSAS, 'DataCollectorId': gatewayID, 'PayloadType': 'Measurements', 'Timestamp': time_now, 'Cache-Control': 'no-cache', 'Content-Length': len(tilt_status) }

response = requests.post(url, data=tilt_status, headers=headers)
