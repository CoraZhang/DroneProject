...
# v2.00 2018/10/06 Fredrik Bolzek
# - Make fleetsim.pl work with new AppIoT environment.
##############################################################################
$| = 1;
...
my $DefaultOpenDoorPhoto = "images/OpenDoors.jpg";
...
{
 if(%Config)
   { &stderr("Logging in to AppIoT gateway $Config{gatewayId}"); }
 else
 ...
 if($Sensor{$sensorname})
   {
    ...
    my $url = $AppIotApiUrl . $Sensor{$sensorname};

    # &debug("AppIoT URL: $url");
    # &debug("command: $command");
    # &debug("headers = ".Dumper($headers));
    
    # Needed initialize the REST::Client (was removed from AppIoT Login)
...
sub updateTruckJsonFile
{
 ...
 if($truck > 0)
   { &log("[$dttm] Truck $truck: $JSON{$truck}{LAT},$JSON{$truck}{LNG}; Accident=$JSON{$truck}{ACCIDENT}; Door=$JSON{$truck}{DOOR}; TempC=$JSON{$truck}{TEMPERATURE}"); }
 else   
   { &stderr("[$dttm] Truck $truck: $JSON{$truck}{LAT},$JSON{$truck}{LNG}; Accident=$JSON{$truck}{ACCIDENT}; Door=$JSON{$truck}{DOOR}; TempC=$JSON{$truck}{TEMPERATURE}"); }
